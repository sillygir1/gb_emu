...
bool test_ADC_A_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if ((uint8_t)system->registers[A] == (uint8_t)res &&
        carry == GET_FLAG(CARRY) && halfcarry == GET_FLAG(HALFCARRY) &&
        zero == GET_FLAG(ZERO) && sub == GET_FLAG(SUB)) {
      if (print_all)
      ...
      printf("ADC A,r8: %hhu %hhu (no carry) failed\nExpected: %u z: %u n: %u "
             ...
             system->registers[A], GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
  for (uint16_t i = 0; i <= 0xFF; i++) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if ((uint8_t)system->registers[A] == (uint8_t)res &&
      ...
      printf("ADC A,[HL]: %hhu %hhu (no carry) failed\nExpected: %u z: %u n: "
             "%u hc: %u c: %u, got: %u z: %u n: %u hc: %u c: %u\n",
             i, test_value, res, zero, sub, halfcarry, carry,
             system->registers[A], GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
...
bool test_CP_A_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if (carry == GET_FLAG(CARRY) && halfcarry == GET_FLAG(HALFCARRY) &&
        zero == GET_FLAG(ZERO) && sub == GET_FLAG(SUB)) {
      if (print_all)
        printf("CP A,r8: %hhu passed\n", i);
    } else {
      printf("CP A,r8: %hhu %hhu failed\nExpected: z: %u n: %u hc: %u c: %u, "
             "got: z: %u n: %u hc: %u c: %u\n",
             i, test_value, zero, sub, halfcarry, carry, GET_FLAG(ZERO),
             GET_FLAG(SUB), GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
  if (!failed) {
...
bool test_DEC_r8(System *system, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if ((uint8_t)system->registers[test_r8_1] == (uint8_t)res &&
      ...
      printf("DEC r8: %hhu failed\nExpected: %u z: %u n: %u hc: %u c: %u, got: "
             "%u z: %u n: %u hc: %u c: %u\n",
             i, res, zero, sub, halfcarry, carry, system->registers[test_r8_1],
             GET_FLAG(ZERO), GET_FLAG(SUB), GET_FLAG(HALFCARRY),
             GET_FLAG(CARRY));
      failed = true;
    }
  }
...
bool test_DEC_HL(System *system, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if ((uint8_t)system->memory[test_address] == (uint8_t)res &&
      ...
      printf("DEC [HL]: %hhu failed\nExpected: %u z: %u n: %u hc: %u c: %u, "
             "got: %u z: %u n: %u hc: %u c: %u\n",
             i, res, zero, sub, halfcarry, carry, system->memory[test_address],
             GET_FLAG(ZERO), GET_FLAG(SUB), GET_FLAG(HALFCARRY),
             GET_FLAG(CARRY));
      failed = true;
    }
...
bool test_SBC_A_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    // if ((i & 0xF + test_value & 0xF) & 0x10 == 0x10) { halfcarry = true; }
...
bool test_ADD_HL_r16(System *system, uint16_t test_value, bool print_all) {
  ...
  for (uint32_t i = 0; i <= 0xFFFF; i++) {
    ...
    if (GET_16BIT_REGISTER(HL) == res && carry == GET_FLAG(CARRY) &&
        halfcarry == GET_FLAG(HALFCARRY) && zero == GET_FLAG(ZERO) &&
        sub == GET_FLAG(SUB)) {
      if (print_all)
      ...
      printf("ADD [HL],r16: %hu %hu failed\nExpected: %u z: %u n: %u hc: %u c: "
             "%u, got: %u z: %u n: %u hc: %u c: %u\n",
             i, test_value, res, zero, sub, halfcarry, carry,
             GET_16BIT_REGISTER(HL), GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (res == system->registers[test_r8_1]) {
      ...
      printf("RES u3,r8: %hhu %hhu failed\nExpected: %u got: %u\n", i,
             test_value, res, system->registers[test_r8_1]);
      failed = true;
    }
  }
  if (!failed) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (res == system->memory[test_address]) {
      ...
      printf("RES u3,[HL]: %hhu %hhu failed\nExpected: %u got: %u\n", i,
             test_value, res, system->registers[A]);
      failed = true;
    }
  }
  if (!failed) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (res == system->memory[test_address]) {
      ...
      printf("SET u3,[HL]: %hhu %hhu failed\nExpected: %u got: %u\n", i,
             test_value, res, system->memory[test_address]);
      failed = true;
    }
  }
  if (!failed) {
...
bool test_SWAP_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  zero = !res;
  if ((uint8_t)system->registers[test_r8_1] == (uint8_t)res &&
      0 == GET_FLAG(CARRY) && 0 == GET_FLAG(HALFCARRY) &&
      zero == GET_FLAG(ZERO) && 0 == GET_FLAG(SUB)) {
    if (print_all)
      printf("SWAP r8: %hhu (no carry) passed\n", test_value);
  } else {
    printf("SWAP r8: %hhu (no carry) failed\nExpected: %u z: %u n: %u hc: %u "
           "c: %u, got: %u z: %u n: %u hc: %u c: %u\n",
           test_value, res, zero, 0, 0, 0, system->registers[test_r8_1],
           GET_FLAG(ZERO), GET_FLAG(SUB), GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
    failed = true;
  }
  if (!failed) {
    if (print_all)
...
bool test_SWAP_HL(System *system, uint8_t test_value, bool print_all) {
  ...
  if ((uint8_t)system->memory[test_address] == (uint8_t)res &&
    ...
    printf("SWAP [HL]: %hhu (no carry) failed\nExpected: %u z: %u n: %u hc: %u "
           "c: %u, got: %u z: %u n: %u hc: %u c: %u\n",
           test_value, res, zero, 0, 0, 0, system->memory[test_address],
           GET_FLAG(ZERO), GET_FLAG(SUB), GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
    failed = true;
  }
  if (!failed) {
...
bool test_RL_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint8_t i = 1; i < 9; i++) {
    ...
    zero = !res;
    if ((uint8_t)system->registers[test_r8_1] == (uint8_t)res &&
        carry == GET_FLAG(CARRY) && 0 == GET_FLAG(HALFCARRY) &&
        zero == GET_FLAG(ZERO) && 0 == GET_FLAG(SUB)) {
      if (print_all)
      ...
      printf("RL r8: %hhu %hhu (no carry) failed\nExpected: %u z: %u n: %u hc: "
             ...
             system->registers[test_r8_1], GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
  ...
  for (uint8_t i = 1; i < 9; i++) {
    ...
    if ((uint8_t)system->memory[test_address] == (uint8_t)res &&
      ...
      printf("RL [HL]: %hhu %hhu (no carry) failed\nExpected: %u z: %u n: %u "
             "hc: %u c: %u, got: %u z: %u n: %u hc: %u c: %u\n",
             i, test_value, res, zero, 0, 0, carry,
             system->memory[test_address], GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
...
bool test_RLA(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint8_t i = 1; i < 9; i++) {
    ...
    if ((uint8_t)system->registers[A] == (uint8_t)res &&
        carry == GET_FLAG(CARRY) && 0 == GET_FLAG(HALFCARRY) &&
        0 == GET_FLAG(ZERO) && 0 == GET_FLAG(SUB)) {
      if (print_all)
        printf("RLA: %hhu (no carry) passed\n", test_value);
    } else {
      printf("RLA: %hhu %hhu (no carry) failed\nExpected: %u z: %u n: %u hc: "
             "%u c: %u, got: %u z: %u n: %u hc: %u c: %u\n",
             i, test_value, res, 0, 0, 0, carry, system->registers[A],
             GET_FLAG(ZERO), GET_FLAG(SUB), GET_FLAG(HALFCARRY),
             GET_FLAG(CARRY));
      failed = true;
    }
...
bool test_SRA_r8(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint8_t i = 1; i < 9; i++) {
    carry = system->registers[test_r8_1] & 1;
    uint8_t res =
        (system->registers[test_r8_1] >> 1) + (system->registers[A] & (1 << 7));
  ...
  for (uint32_t i = 0; i <= 0xFFFF; i++) {
    ...
    if ((uint16_t)GET_16BIT_REGISTER(test_r16) == (uint16_t)res) {
      ...
      printf("LD r16,n16: %hhu %hhu failed\nExpected: %u, got: %u\n", i,
             test_value, res, GET_16BIT_REGISTER(test_r16));
      failed = true;
    }
  }
  if (!failed) {
...
bool test_LD_HLI_A(System *system, uint8_t test_value, bool print_all) {
  ...
  for (uint16_t i = 0; i <= 0xFF; i++) {
    ...
    if ((uint8_t)system->memory[test_address] == (uint8_t)res &&
      ...
      printf("LD [HLI],A: %hhu %hhu failed\nExpected: %u, HL: %x got: %u, HL: "
             "%x\n",
             i, test_value, res, hl, system->memory[test_address],
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    if (GET_16BIT_REGISTER(PC) == res) {
      ...
      printf("JP сс,n16: %hhu %hu failed\nExpected: %u got: %u\n", i,
             test_value, res, GET_16BIT_REGISTER(PC));
      failed = true;
    }
  }
  if (!failed) {
...
bool test_ADD_SP_e8(System *system, uint16_t test_value, bool print_all) {
  ...
  for (int i = -128; i <= 127; i++) {
    ...
    if (GET_16BIT_REGISTER(SP) == res && carry == GET_FLAG(CARRY) &&
        halfcarry == GET_FLAG(HALFCARRY) && zero == GET_FLAG(ZERO) &&
        sub == GET_FLAG(SUB)) {
      if (print_all)
      ...
      printf("ADD SP,e8: %d %hu failed\nExpected: %u z: %u n: %u hc: %u c: %u, "
             "got: %u z: %u n: %u hc: %u c: %u\n",
             i, test_value, res, zero, sub, halfcarry, carry,
             GET_16BIT_REGISTER(SP), GET_FLAG(ZERO), GET_FLAG(SUB),
             GET_FLAG(HALFCARRY), GET_FLAG(CARRY));
      failed = true;
    }
  }
...
bool test_LD_SP_n16(System *system, uint16_t test_value, bool print_all) {
  ...
  if (GET_16BIT_REGISTER(SP) == test_value) {
    ...
    printf("LD SP,n16: %hhu failed\nExpected: %u, got: %u\n", test_value,
           test_value, GET_16BIT_REGISTER(SP));
    failed = true;
  }
  if (!failed) {
    if (print_all)
...
bool test_LD_n16_SP(System *system, uint16_t test_value, bool print_all) {
  ...
  uint16_t loaded =
      system->memory[test_address] + system->memory[test_address + 1] << 8;
  if (GET_16BIT_REGISTER(SP) == test_value) {
    if (print_all)
      printf("LD SP,n16: %hhu passed\n", test_value);
  } else {
...
bool test_CCF(System *system, bool print_all) {
  ...
  if (GET_FLAG(CARRY) == carry && GET_FLAG(SUB) == 0 &&
      GET_FLAG(HALFCARRY) == 0) {
    if (print_all)
      printf("CCF: passed\n");
  } else {
...
bool test_CPL(System *system, uint8_t test_value, bool print_all) {
  ...
  if (system->registers[A] == res && GET_FLAG(SUB) == 1 &&
      GET_FLAG(HALFCARRY) == 1) {
    if (print_all)
      printf("CPL: passed\n");
  } else {
    printf(
